...
import { UserDevice, AvailableDevice } from '../types';
...
export function FloorMap() {
  ...
  const handleMapClick = async (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    try {
      const available = await userDeviceService.getAvailableDevices();
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Container do Mapa com Scroll Horizontal se necessário */}
      <div className="relative w-full overflow-x-auto rounded-xl border border-border bg-muted/30 shadow-inner no-scrollbar">
        ...
        <div 
          ...
          style={{
            backgroundImage: `url('/mapa-producao.png')`, // <-- Coloque a imagem gerada na pasta public
        ...
        >
          ...
          <div className="absolute top-6 left-6 z-10 flex flex-col gap-1">
            <div className="flex items-center gap-2 bg-background/80 backdrop-blur-md px-3 py-1.5 rounded-lg border border-border shadow-sm">
              ...
              <h2 className="text-sm font-bold text-foreground tracking-tight">PLANTA INDUSTRIAL A1</h2>
            </div>
            {loading && <span className="text-[10px] text-brand-blue animate-pulse ml-1 font-mono">SYNCING NODES...</span>}
          </div>
          ...
          {devices.map((device) => (
            <div 
              ...
              style={{ left: `${device.coordinateX}%`, top: `${device.coordinateY}%` }}
              onClick={(e) => {
                e.stopPropagation();
                router.push(`/dashboard/devices/${device.deviceId}`);
              }}
            >
              {/* Ping de Status Animado */}
              <div className="relative">
                <div className={`absolute inset-0 rounded-full animate-ping opacity-20 ${
                  device.status === 'ONLINE' ? 'bg-green-500' : 'bg-red-500'
                }`} />
                <div className={`w-3.5 h-3.5 rounded-full border-2 border-white dark:border-slate-900 shadow-lg transition-all group-hover:scale-150 ${
              ...
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-card/95 backdrop-blur-md border border-border p-3 rounded-lg text-xs whitespace-nowrap z-30 shadow-2xl pointer-events-none">
                ...
                <p className="text-muted-foreground font-mono text-[10px]">{device.macAddress}</p>
                <div className="mt-2 pt-2 border-t border-border flex justify-between gap-4">
                    <span className="text-[10px] text-brand-blue uppercase font-bold tracking-tighter">Clique para ver logs</span>
                </div>
              </div>
            </div>
          ))}

          {/* Feedback Visual ao clicar (Preview do local) */}
          {modalOpen && (
            <div 
      ...
      {/* Modal de Cadastro (Adaptado para o Tema) */}
      {modalOpen && (
        <>
          <div className="fixed inset-0 bg-background/60 backdrop-blur-sm z-40 transition-opacity" onClick={() => setModalOpen(false)} />
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-95 bg-card border border-border rounded-2xl shadow-2xl z-50 p-6 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">Configure o sensor no ponto selecionado</p>
              </div>
              <button onClick={() => setModalOpen(false)} className="p-2 hover:bg-muted rounded-full transition-colors text-muted-foreground hover:text-foreground">
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleSave} className="space-y-5">
              <div className="space-y-2">
                <label className="text-[11px] text-muted-foreground uppercase font-black tracking-widest block">Dispositivo Disponível</label>
                <select 
              ...
              <div className="space-y-2">
                <label className="text-[11px] text-muted-foreground uppercase font-black tracking-widest block">Identificação na Planta</label>
                <input 
                  type="text" 
              ...
              <div className="flex items-center gap-2 p-3 bg-brand-blue/5 rounded-lg border border-brand-blue/10">
                ...
                <span className="text-[11px] text-brand-blue font-mono font-bold">
                    COORDENADAS: {clickPos.x.toFixed(1)}% X {clickPos.y.toFixed(1)}%
                </span>
              </div>
              ...
              >
                {saving ? 'PROCESSANDO...' : <><Save size={18} /> CONFIRMAR VÍNCULO</>}
              </button>
            </form>
          </div>
        </>
